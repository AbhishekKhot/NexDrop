// Backend configuration. Only RELAY_* constants are live in the current
// Remote-only build. LAN constants are line-commented below — re-enable in
// step with the LAN files under backend/src/{api,discovery,transport,chunking,crypto}.
...
// `PORT` is the fallback so PaaS platforms (Render, Fly, Heroku) that inject
// a dynamic port work without extra config. `RELAY_PORT` still wins if set.
export const RELAY_PORT = parseInt(
  process.env.RELAY_PORT ?? process.env.PORT ?? "4002",
  10,
);
